...
class Backfill:
    def __init__(self, mode = 0, ad_mode = 0, node_module = None,bb_module=None, debug = None, para_list = None):
    ...
    def backfill_EASY(self):
        ...
        self.node_module.reserve(self.wait_job[0]['proc'],self.wait_job[0]['bb'], self.wait_job[0]['index'], self.wait_job[0]['run'])
        i = 1
        job_num = len(self.wait_job)
        while (i < job_num):
            backfill_test = 0
            backfill_test = self.node_module.pre_avail(self.wait_job[i]['proc'],self.wait_job[i]['bb'],\
                    self.current_para['time'], self.current_para['time']+self.wait_job[i]['run'])
            if (backfill_test == 1):
                backfill_list.append(self.wait_job[i]['index'])
                self.node_module.reserve(self.wait_job[i]['proc'], self.wait_job[i]['bb'],self.wait_job[i]['index'], self.wait_job[i]['run'])
            i += 1
